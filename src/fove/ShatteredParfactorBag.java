...
public class ShatteredParfactorBag {
    ...
    /**
     * Create a new ShatteredParfactorBag, with the same probability
     * distribution as the argument set.
     */
    public ShatteredParfactorBag(Set<Parfactor> parfactors) {
        ...
        for (Parfactor pf: parfactors) add(pf);
    }
    ...
    public void addAll(Collection<Parfactor> pfs) {
        Stack<Parfactor> s = new Stack<Parfactor>();
        for (Parfactor pf: pfs) s.push(pf);
        addAll(s);
    }
    ...
    private void addAll(Stack<Parfactor> stack) {
        ...
        /* postcondition:
         *
         * - shatteredParfactors is shattered; grounds to the union of its
         *   current grounding with the grounding of all parfactors in queue.
         * 
        ...
        while (!stack.empty()) {
            ...
            if (DEBUG) System.out.println("Got dirty parfactor:\n\t" + dirty);
            ...
            outer_for: for (int i = 0; i < terms.size(); i++) {
                ...
                Set<Parfactor.TermPtr> cleanTerms =
                    termsWithSameSignature(terms.get(i));
                
                if (cleanTerms.isEmpty()) continue;
			...
			continue; // already removed due to earlier term
                    ...
                    if (cpf.shatter(dirty, cIndex, i, residuals)) {
                        ...
                        remove(cpf);
                        for (Parfactor p: residuals) stack.push(p);
        ...
        if (DEBUG) {
            if (!isShattered()) throw new IllegalStateException();
            System.out.println("----------------");
    ...
    /**
     * Returns an unmodifiable collection view of the parfactors in the
     * bag.
     */
    ...
    public void splitOnQueryTerms(Collection<? extends Term> queryTerms) {
		...
		for (Parfactor.TermPtr tPtr: cleanTerms) {
			...
			foundSplit = true;
			remove(pf); 
			for (Parfactor res: residuals) add(res);
		    }
		}
	    } 
        }
        if (DEBUG) {
            if(!isShattered(queryTerms)) throw new IllegalStateException();
        }
    }
    ...
    private void indexFunctionSignatures(Parfactor pf) {
        ...
        if (DEBUG) System.out.println("Indexing:\n\t" + pf);
        ...
        if (DEBUG) System.out.println("... done indexing.");
    }
    ...
    private Function.Sig termSignature(Term t) {
        ...
        if (t instanceof FuncAppTerm) {
            ...
            throw new IllegalArgumentException
                ("can only handle CountingTerms and FuncAppTerms,"
    ...
    private boolean isShattered(Collection<? extends Term> queryTerms) {
        ...
        if (DEBUG) System.out.println(" shattered!");
        return true;
    }
