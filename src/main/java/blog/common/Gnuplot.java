...
public class Gnuplot {
  ...
  public static class YSeries {
    public YSeries(List<?> data) {
      this.directives = new LinkedList<String>();
      this.data = Util.getIteratorNullaryFunction(data);
    }

    public YSeries(List<String> directives, List<?> data) {
      this.directives = directives;
      this.data = Util.getIteratorNullaryFunction(data);
    }

    public YSeries(List<String> directives, NullaryFunction data) {
      this.directives = directives;
      this.data = data;
    }
  ...
  public static void main(String[] args) {
    plot(Arrays.asList( // precommands
        "persist",
        // "set term postscript color",
        // "set output 'test.ps'",
        "set xlabel 'Time'", "set ylabel 'Intensity'"),
        Util.getIteratorNullaryFunction(Arrays.asList(10, 20, 30, 40)), // xSeries
        Arrays.asList(
        // list of ySeries
            new YSeries(Arrays.asList("title 'random sequence 1'",
                "w linespoints"), Arrays.asList(1, 2, 4, 3)), new YSeries(
                Arrays.asList("title 'random sequence 2'", "w linespoints"),
