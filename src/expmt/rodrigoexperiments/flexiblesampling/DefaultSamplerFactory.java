 ...
 * a partial world and a set of variables in the process of being instantiated.
...
public class DefaultSamplerFactory implements SamplerFactory {
	...
	public Sampler make(VarWithDistrib seedVar, PartialWorld world,
		...
		for (Class importanceSamplerClass : importanceSamplerClasses) {
			try {
				Method applies = importanceSamplerClass.getMethod("applies",
						new Class[] { VarWithDistrib.class, PartialWorld.class, Set.class,
								SamplerFactory.class });
				Sampler sampler = (Sampler) applies.invoke(null, seedVar, world,
						beingExternallyInstantiatedVars, this);
				if (sampler != null)
					return sampler;
			} catch (Exception e) {
				throw new Error("Could not invoke 'applies' for "
						+ importanceSamplerClass, e);
			}
		}
		return null;
	}
