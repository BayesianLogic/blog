 ...
 * initial world by a {@link LWSampler}.
 * The user must implement {@link #proposeNextState(PartialWorldDiff)}.
 */
public abstract class AbstractProposer implements Proposer {
    ...
    /**
     * Actually builds initial state, after whatever initializations
     * are necessary.  Separated from {@link #initialize(Evidence,List)} 
     * so extensions can change it alone.
     */
	...
	while (true) {
		...
		PartialWorld underlying 
		    = new DefaultPartialWorld(initWorld.getIdTypes());
