...
public class TupleSetSpecEvidenceLikelihoodWeighterTest extends TestCase {
	...
	assertLikelihood(modelDescription, evidenceDescription, world, expectedLikelihood);
	...
	expectedLikelihood = gaussianObservationsLikelihood(
		Util.arrayList(1.0,2.0,3.0),
		variance,
		Util.arrayList(1.0,2.0,3.0));
	assertLikelihood(modelDescription, evidenceDescription, world, expectedLikelihood);

	/** The same as previous, but with smaller variance and thus higher likelihood. */
	variance = 1.0;
	modelDescription =
	    "type Blip;" +
	    "guaranteed Blip b1, b2, b3;" +
	...
	evidenceDescription = "obs {ApparentPos(b) for Blip b} = {null for Blip b};"; // the 'null' is for forcing the right hand side to be a tuple set, rather than an explicit set. We should eventually make BLOG represent tuple sets as regular sets the elements of which are tuples to avoid problems like that.
	world = new DefaultPartialWorld();
	expectedLikelihood = gaussianObservationsLikelihood(
		Util.arrayList(),
		variance,
    ...
    public static double gaussianObservationsLikelihood(ArrayList means, double variance, ArrayList observations) {
	if (observations.size() < means.size())
	    return 0;
	// See and reflect changes in (*) in {@link TupleSetSpecEvidenceLikelihoodWeighter}.
	...
	// Goes through all associations from gaussians to values, including those that do not map to all values,
	// which are skipped.
	// This is not the most efficient way of generating those.
	double sum = 0;
	List observationIndices = Util.makeIntegerSequenceList(0, observations.size());
	...
	while (associationIt.hasNext()) {
		...
		double factor = gaussians.get(i).getProb(((Number)observations.get(valueIndex)).doubleValue());
		product *= factor;
	    }
	    sum += product;
	}
