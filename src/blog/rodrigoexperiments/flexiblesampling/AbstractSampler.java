...
public abstract class AbstractSampler implements Sampler {
    ...
    public AbstractSampler(VarWithDistrib seedVar, PartialWorld world, Set beingExternallyInstantiatedVars, SamplerFactory fallbackFactory) {
	...
	uninstantiateBlockVarsAfterMeasuringWhenUnsampling(); // uninstantiated at end only because subsequent "samplings" may depend on value of previous ones.
	return sample;
    }
	...
	context = new EvalContext(world, beingExternallyInstantiatedVars, fallbackFactory);
	sampleStory();
    ...
    protected void setValueStoryPoint(VarWithDistrib var, DependencyModel.Distrib proposal) {
	Object value;
	if (instantiating) {
	...
	if (instantiating)
	    var.ensureDetAndSupported(context); // seedVar already supported when uninstantiating.
