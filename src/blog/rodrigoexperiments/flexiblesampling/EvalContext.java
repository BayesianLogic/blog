...
public class EvalContext extends DefaultEvalContext implements InstantiatingEvalContext {
    public EvalContext(PartialWorld world, Set beingExternallyInstantiatedVars, SamplerFactory fallbackFactory) {
	super(world);
	...
	if (value == null) {
		...
		throw new IllegalArgumentException("Don't know how to instantiate: " + var);
	    }
	}
	return value;
    }
	...
	if (beingExternallyInstantiatedVars.contains(var))
	    throw new Error("Instantiation of " + var + " requested in order to instantiate itself.");// TODO: maybe a good idea to tell which samplers requested both times.
	Sampler fallback = fallbackFactory.make(var, world, beingExternallyInstantiatedVars);
	if (fallback == null)
	    throw new Error("Fallback of " + this + " does not know how to sample " + var + " in " + world);
	Sample fallbackResult = fallback.sample();
	weight *= fallbackResult.getWeight(); // bring in whatever importance sampling went on there.
