 ...
 * regarding temporal aspects (DBLOG)
 * (general ones are in {@link BLOGUtil}.
 * @author Rodrigo
 */
public class DBLOGUtil {
    /**
     * Identifies the largest time step in a world and
     * uninstantiates all temporal random variables with a different time step. 
     */
	...
	if (largestTimestepIndex != -1)
	    uninstantiateAllTemporalsWithAnIndexDifferentFrom(largestTimestepIndex, world);
    }
    
    /** 
     * Returns a string obtained by replacing all identifiers <code>t</code> in a given string by
     * the string representation of a timestep the index of which is given.
     */
    public static String replaceTByTimeStep(String string, int timestepIndex) {
	return string.replaceAll("\\bt\\b", "@" + timestepIndex);
    }

    /** Returns the query obtained by instantiating a query string with a given time step. */ 
    public static ArgSpecQuery getQueryForTimestep(String queryString, Model model, int timestepIndex) {
	String queryForLastestTimestepString = DBLOGUtil.replaceTByTimeStep(queryString, timestepIndex);
	ArgSpecQuery query = 
	    BLOGUtil.parseQuery_NE("query " + queryForLastestTimestepString + ";", model);
	return query;
    }
    ...
    public static void uninstantiateAllTemporalsWithAnIndexDifferentFrom(int largest, PartialWorld world) {
    ...
    /**
     * Returns the single timestep in the given argspec,
     * or <code>null</code> if there is none in it,
     * or exits if there are more than one. 
     */
    public static Timestep getSingleTimestepIn(ArgSpec argSpec) {
        Collection timesteps = getTimestepTermsIn(argSpec, new LinkedList());
        if (timesteps.size() > 1)
            Util.fatalError("DBLOGUtil.getTimestep called for argspec with more than one timestep: " + argSpec);
        if (timesteps.size() == 0)
            return null;
    ...
    /** Returns a Timestep object if this is a constant timestep term, or null otherwise. */
	...
	if ( ! (funcAppTerm.getFunction() instanceof NonRandomFunction) )
	    return null;
	NonRandomFunction nonRandomFunction = (NonRandomFunction) funcAppTerm.getFunction();
	if ( ! (nonRandomFunction.getInterpretation() instanceof ConstantInterp) )
	    return null;
	ConstantInterp interp = (ConstantInterp) nonRandomFunction.getInterpretation();
	...
	public boolean evaluate(Object term) { return isTimestep((ArgSpec) term); }
    };

    private static final TimestepSelector timestepSelector = new TimestepSelector();

    /** Adds all timesteps in an ArgSpec to a collection and return that collection. */
    public static Collection getTimestepTermsIn(ArgSpec argSpec, Collection timesteps) {
    ...
    private static class TimestepTermComparator implements Comparator {
	 public int compare(Object o1, Object o2) {
	     return getTimestepInTimestepTerm((ArgSpec)o1).compareTo(getTimestepInTimestepTerm((ArgSpec)o2));
	 }
    ...
    /**
     * Returns a list of Evidence objects, equivalent to a given Evidence object, ordered by maximum timestep present in their statements.
     * (an absence of timesteps places the statement in the first Evidence object).
     */
	...
	for (it = evidence.getValueEvidence().iterator(); it.hasNext(); ) {
	    ...
	    TreeSet timesteps = (TreeSet) getTimestepTermsIn(statement.getLeftSide(), new TreeSet(new TimestepTermComparator()));
	...
	for (it = evidence.getSymbolEvidence().iterator(); it.hasNext(); ) {
	    ...
	    TreeSet timesteps = (TreeSet) getTimestepTermsIn(statement.getSetSpec(), new TreeSet(new TimestepTermComparator()));
	    Object maxTimestep = timesteps.isEmpty()? null : timesteps.last();
	    statementsByTimestep.add(maxTimestep, statement);
	}
	...
	result.add(Evidence.constructAndCompile((Collection)statementsByTimestep.get(null)));
	statementsByTimestep.remove(null);
    }

    private static void addTemporalEvidenceToListInTimestepOrder(final List result,
            HashMultiMap statementsByTimestep) {
	applyProcedureToEvidenceInTimestepOrderUpTo(-1, statementsByTimestep, new UnaryProcedure() { public void evaluate(Object evidenceObj) {
    ...
    private static void applyProcedureToEvidenceInTimestepOrderUpTo(
        ...
        for (Iterator it = sortedKeys.iterator(); it.hasNext(); ) {
            ...
            if (timestepIndex != -1 && getTimestepInTimestepTerm(key).getValue() > timestepIndex) break;
            Evidence evidence = Evidence.constructAndCompile((Collection)statementsByTimestep.get(key));
	...
	HashMultiMap statementsByTimestep = getStatementsByTimestep(evidence);
	applyProcedureToEvidenceInTimestepOrderUpTo(timestepIndex, statementsByTimestep, new UnaryProcedure() { public void evaluate(Object evidenceObj) {
