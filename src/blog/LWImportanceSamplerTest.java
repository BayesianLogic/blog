...
public class LWImportanceSamplerTest extends TestCase {
	...
	String description =
	    ...
	    "type Dummy;" +
	    "guaranteed Dummy d;" +
	    "query {Source(B1), Source(B2), Source(B3) for Dummy d};" + // dummy makes it a tuple instead of a set

	    "query Source(B1);" +
	    "query Source(B2);" +
	    "query Source(B3);";

	ModelEvidenceQueries meq = BLOGUtil.parseModelEvidenceQueries_NE(description);
	InferenceEngine sampling = new SamplingEngine(
		meq.model, Util.properties(
	...
	assertEquals(
		...
		probDry,
		errorMargin);
	double probRainy = BLOGUtil.getProbabilityByString(query1, meq.model, "Rainy");
	assertEquals(
		0.28,
		probRainy,
		errorMargin);
	...
	assertEquals(
		probOf1,
		BLOGUtil.getProbabilityByString(query, meq.model, "1.0"),
		errorMargin);
	assertEquals(
		probOf2,
		BLOGUtil.getProbabilityByString(query, meq.model, "2.0"),
		errorMargin);
	assertEquals(
		probOf3,
		BLOGUtil.getProbabilityByString(query, meq.model, "3.0"),
		errorMargin);
    }
