...
public class DistanceTest extends TestCase {
	...
	public void testDistanceListList() {
		...
		assertEquals(Math.sqrt(2.0),
				distance.distance(Util.list(0.0, 0.0), Util.list(1.0, 1.0)), 0.000001);
		assertEquals(0.0,
				distance.distance(Util.list(1.0, 1.0), Util.list(1.0, 1.0)), 0.000001);
	}
	...
	public void testApproximateDistance() {
		...
		assertEquals(2 * Math.sqrt(2.0),
				distance.approximateDistanceOfSets(s1, s2), 0.000001);

		s1 = Util.set(Util.list(1.0, 1.0), Util.list(2.0, 2.0));
		s2 = Util.set(Util.list(3.0, 3.0), Util.list(4.0, 4.0));
		assertEquals(4 * Math.sqrt(2.0),
				distance.approximateDistanceOfSets(s1, s2), 0.000001);
	}
	...
	public void testDistanceObjectCollectionUnaryFunction() {
		...
		set = Util.set(Util.list(0.0, 0.0, 3.0), Util.list(0.0, 0.0, 1.0),
				Util.list(0.0, 0.0, 2.0));
		weightFunction = new UnaryFunction() {
	...
	public void testDistanceCollectionUnaryFunctionCollectionUnaryFunction() {
		...
		s1 = Util.set(Util.list(0.0, 0.0, 1.0), Util.list(0.0, 0.0, 2.0),
				Util.list(0.0, 0.0, 3.0));
		weightFunction1 = new UnaryFunction() {
			public Object evaluate(Object o) {
				return 1.0;
			}
		};

		s2 = Util.set(Util.list(0.0, 0.0, 3.0), Util.list(0.0, 0.0, 1.0),
				Util.list(0.0, 0.0, 2.0));
		weightFunction2 = new UnaryFunction() {
			public Object evaluate(Object o) {
				return 2.0;
			}
		};

		assertEquals((1.0 + 1.0 + 2.0 / 3) / 3,
				distance.distance(s1, weightFunction1, s2, weightFunction2), 0.0000001);
	}
