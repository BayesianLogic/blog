...
public class SymbolEvidenceLikelihoodWeighterTest extends TestCase {
	...
	"" +
	"random Blip B1;" +
	"B1 ~ UniformChoice({Blip b});" +
	"" +
	"random Blip B2;" +
	"B2 ~ UniformChoice({Blip b});" +
	"" +
	...
	ValueEvidenceStatement st1 = BLOGUtil.parseValueEvidenceStatement_NE("obs ApparentPos(B1)=3;", model);
	ValueEvidenceStatement st2 = BLOGUtil.parseValueEvidenceStatement_NE("obs ApparentPos(B2)=4;", model);
	ValueEvidenceStatement st3 = BLOGUtil.parseValueEvidenceStatement_NE("obs ApparentPos(B1)=ApparentPos(B2);", model);
	ValueEvidenceStatement st4 = BLOGUtil.parseValueEvidenceStatement_NE("obs ApparentPos(B1)=4;", model);
	ValueEvidenceStatement st5 = BLOGUtil.parseValueEvidenceStatement_NE("obs ApparentPos(b)=4;", model);
	...
	evidenceByTerm = SymbolEvidenceLikelihoodWeighter.getValueEvidenceByLastIdentifierToBeSampled(terms, evidence);
	...
	// statements with more than one identifier get assigned to the latest of them.
	...
	assertEquals(0.053990966513188056, likelihoodAndWeight.likelihood); // density of Gaussian(3,1) at 1.
	assertEquals(1.0, likelihoodAndWeight.weight);

	// Test likelihood zero when world does not correspond to number of symbols in evidence.
	evidence = BLOGUtil.parseEvidence_NE("obs {Aircraft air} = {A1, A2, A3};", model);
	world = new DefaultPartialWorld();
	ses = new SymbolEvidenceLikelihoodWeighter();
	likelihoodAndWeight = ses.likelihoodAndWeight(evidence, world);
	assertEquals(0.0, likelihoodAndWeight.likelihood); // since model defines two aircraft.
	assertEquals(1.0, likelihoodAndWeight.weight);

	// Test probability zero for equality of continuous variables.
	model = Model.readFromString(modelDescription); // avoids symbol redefinition error.
	...
	ses = new SymbolEvidenceLikelihoodWeighter();
	likelihoodAndWeight = ses.likelihoodAndWeight(evidence, world);
	// fall back will sample continuous variables, with probability zero of them turning out equal, and so likelihoods will be zero with weight 1.
	assertEquals(0.0, likelihoodAndWeight.likelihood);
	assertEquals(1.0, likelihoodAndWeight.weight);

//	Gaussian g3 = new Gaussian(Util.list(3, 1));
//	System.out.println("Gaussian likelihood for 1 given 3: " + g3.getProb(1));
//	System.out.println("Gaussian likelihood for 6 given 3: " + g3.getProb(6));
//	Gaussian g7 = new Gaussian(Util.list(7, 1));
//	System.out.println("Gaussian likelihood for 1 given 7: " + g7.getProb(1));
//	System.out.println("Gaussian likelihood for 6 given 7: " + g7.getProb(6));

	// The first test of a case in which association sampling actually occurs.
	model = Model.readFromString(modelDescription); // avoids symbol redefinition error.
	world = new DefaultPartialWorld();
	world.setValue((BasicVar)BLOGUtil.parseVariable_NE("Position(a)", model),  new Integer(3));
	world.setValue((BasicVar)BLOGUtil.parseVariable_NE("Position(a2)", model), new Integer(7));
	evidence = BLOGUtil.parseEvidence_NE(
		"obs {Blip x : Source(x) != null} = {Blip1, Blip2};" +
		"obs ApparentPos(Blip1) = 1;" +
		"obs ApparentPos(Blip2) = 6;", model);
	ses = new SymbolEvidenceLikelihoodWeighter();
	likelihoodAndWeight = ses.likelihoodAndWeight(evidence, world);
	
	assertEquals(0.013064233284684921, likelihoodAndWeight.likelihood, 10E-8); // same as before * 0.7
	assertEquals(0.5000000562675874, likelihoodAndWeight.weight, 10E-8);
	// This weight actually depends on the fact that Blip1 is proposal-sampled before Blip2.
	...
	ses = new SymbolEvidenceLikelihoodWeighter();
	likelihoodAndWeight = ses.likelihoodAndWeight(evidence, world);
	// System.out.println("Likelihood: {" + likelihoodAndWeight.likelihood + ", " + likelihoodAndWeight.weight + "}");
	
	assertEquals(0.009144963299279446, likelihoodAndWeight.likelihood, 10E-8); // verified by looking at trace
	assertEquals(0.5000000562675874, likelihoodAndWeight.weight, 10E-8);
	// This weight actually depends on the fact that Blip1 is proposal-sampled before Blip2.
	// The implementation does sample identifiers in the order they are defined.
    }
	...
	// This allows us to test situations where there are more than one identifier in a value evidence statement.
	...
	assertEquals(0.02533029591058445, likelihoodAndWeight.likelihood, 10E-8); // verified by looking at program's trace
	assertEquals(0.08343120942571122, likelihoodAndWeight.weight, 10E-8); // this makes sense since there are (only) two possible good matches.
    }
