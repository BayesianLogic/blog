...
/** 
 * Compares the particle filter and likelihood weighting
 * by running a temporal query with increasing numbers of samples or particles
 * and keeping track of the results variance.
 * We should see a faster decrease in variance for the particle filtering.
 * 
 * The query is hard-coded for the PF and in a file for LW
 * (examples/ParticleFilteringVsLikelihoodWeightingMay2008Experiment.eqblog)
 * (in the latter case, we are simply doing BLOG inference from files).
 * The reason it is hard coded for PF is that reading a query from a file
 * would present the query as a whole at once to the algorithm
 * (this is the normal BLOG semantic for them),
 * but PF takes them one time step at a time and has an API suited for that,
 * which is used here.
 * 
...
public class ParticleFilteringVsLikelihoodWeightingMay2008Experiment {
    ...
    private static String queryValueString = "2"; // The query value whose probability is being computed.
	...
	for (int currentNumberOfSamplesOrParticles = initialNumSamplesOrParticles; currentNumberOfSamplesOrParticles != finalNumSamplesOrParticles; currentNumberOfSamplesOrParticles += incrementNumSamplesOrParticles) {
	    double lwVariance = collectVariance(numTrials, currentNumberOfSamplesOrParticles, probFromLWTrialFunction);
	    double pfVariance = collectVariance(numTrials, currentNumberOfSamplesOrParticles, probFromParticleFilterTrialFunction);
	    Object[] result = {currentNumberOfSamplesOrParticles, pfVariance, lwVariance}; 
	    results.add(result);
	}
	return results;
    }
    ...
    /**
     * Answers the designated query a certain number of trials
     * using a given function
     * and returns the answers variance.
     */
    private static double collectVariance(int numTrials, int currentNumberOfSamplesOrParticles, UnaryFunction probFromTrial) throws Exception {
    ...
    private static double probFromParticleFilterTrial(int currentNumberOfSamplesOrParticles) throws Exception {
	Model model = new Model();
	Collection filenames = new LinkedList();
	filenames.add(modelFilename);
	Main.setup(model, new Evidence(), new LinkedList(), Main.makeReaders(filenames), new LinkedList(), false, false);
	ParticleFilter particleFilter = 
	    new ParticleFilter(model,
	...
	return valueProbability(particleFilter, model, queryString, queryValueString);
    }

    private static double probFromLWTrial(int currentNumberOfSamplesOrParticles) throws Exception {
	Model model = new Model(); 
	...
	Main.setup(model, evidence, queries, Main.makeReaders(filenames), new LinkedList(), false /* verbose */, false /* parseFromMessage */);
	InferenceEngine lw = new SamplingEngine(model, getSamplingProperties(currentNumberOfSamplesOrParticles));
    ...
    static private Properties getParticleFilterProperties(int currentNumberOfSamplesOrParticles) {
	Properties properties = new Properties();
	properties.setProperty("numParticles", Integer.toString(currentNumberOfSamplesOrParticles));
	properties.setProperty("useDecayedMCMC", "true");
	properties.setProperty("numMoves", "1");
	return properties;
    }

    static private Properties getSamplingProperties(int currentNumberOfSamplesOrParticles) {
	Properties properties = new Properties();
	properties.setProperty("numSamples", Integer.toString(currentNumberOfSamplesOrParticles));
	return properties;
    }

    static private void takeStep(
	    ParticleFilter particleFilter,
	    Model model,
    ...
    static private double valueProbability(ParticleFilter particleFilter, Model model, String queryString, String valueString) throws Exception {
	ModelEvidenceQueries meq = BLOGParser.parseString(model, queryString);
	particleFilter.answer(meq.queries);
	return valueProbability(meq.queries, valueString);
    }

    private static double valueProbability(List queries, String valueString) throws Exception {
    ...
    private static UnaryFunction probFromParticleFilterTrialFunction =
	new UnaryFunction() { public Object evaluate(Object currentNumberOfSamplesOrParticlesObject) {
	    try {
		int currentNumberOfSamplesOrParticles =
		    	((Integer)currentNumberOfSamplesOrParticlesObject).intValue();
    ...
    private static UnaryFunction probFromLWTrialFunction =
	new UnaryFunction() { public Object evaluate(Object currentNumberOfSamplesOrParticlesObject) {
	    try {
		int currentNumberOfSamplesOrParticles =
		    	((Integer)currentNumberOfSamplesOrParticlesObject).intValue();
