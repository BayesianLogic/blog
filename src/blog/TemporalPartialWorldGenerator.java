...
/**
 * Generates and increasingly augments a partial world
 * by sampling self-supporting
 * instantiations to support a set of queries at each time step.
 * The set of queries is determined by a given collection of query templates
 * that gets instantiated at each time step.
 * Each template is a string where an identifier "t" stands for the current time step.
 * This is replaced by the actual time step in order to generate the queries that
 * will be supported (thus driving the augmentation of the partial world).
 * @author Rodrigo
 */
public class TemporalPartialWorldGenerator {
    ...
    /**
     * Augments and returns the current world based on the instantiation
     * of the query templates for the time step next to the last one used,
     * or 0 if this is the first generation.
     */
    public void moveOn() {
	moveOn(++lastTimeStep);
    }
    ...
    /**
     * Augments and returns the current world based on the instantiation
     * of the query templates for the given value of t.
     */
	...
	for (Iterator it = queries.iterator(); it.hasNext(); ) {
	    ...
	    BLOGUtil.ensureDetAndSupported(
		    query.getVariable(), currentPartialWorld);
	}
	latestQueries = queries;
	if (afterMove != null) afterMove.evaluate(latestQueries);
    ...
    /**
     * If present, this function is evaluated on the query map (indexed by templates)
     * right after it is supported in the current world.
     */
    public UnaryProcedure afterMove = null;

    public static void main(String[] args) {
	Util.initRandom(true);
	Model model = Model.readFromFile("examples/ParticleFilteringVsLikelihoodWeightingMay2008Experiment.mblog");
	final TemporalPartialWorldGenerator gen =
	    new TemporalPartialWorldGenerator(model, Util.list("query #{Blip r: Time(r) = t};"));
	
	final ArgSpecQuery aircraft = (ArgSpecQuery) BLOGUtil.parseQuery_NE("query #{Aircraft a};", model);

	gen.afterMove = new UnaryProcedure() { public void evaluate(Object queriesObj) {
