...
/**
 * ParticleFilterRunnerOnGenerator extends {@link #ParticleFilterRunner}
 * in order to solve the problem of how to generate evidence for it.
 * This is done by obtaining evidence from a {@link TemporalEvidenceGenerator} .
 * The user must provide a set of link variables that are read from the generator
 * as a query and provided to the particle filter as evidence.
 * The user must also provide a (possibly empty) set of queries
 * to be monitored (distribution printed to standard output)
 * in both generator and particle filter.
 * @author Rodrigo
 */
public class ParticleFilterRunnerOnGenerator extends ParticleFilterRunner {

    public ParticleFilterRunnerOnGenerator(
	    Model model,
	...
	evidenceGenerator = new TemporalEvidenceGenerator(model, linkStrings, queryStrings);
    ...
    private UnaryProcedure afterMoveForward = new UnaryProcedure() { public void evaluate(Object queriesObj) {
	afterMove.evaluate(queriesObj);
    }};

    /** Default afterMove event. */
    private UnaryProcedure monitorGeneratorWorld = new UnaryProcedure() { public void evaluate(Object queriesObj) {
	...
	for (Iterator it = queries.iterator(); it.hasNext(); ) {
	    ...
	    System.out.println(
		    "Value of " + generatorQuery.toString().replaceAll("DerivedVar ", "") + " in generator world: " +
		    getCurrentPartialWorld().getValue(queryVar));
	}
    }};

    protected void beforeEvidenceAndQueries() {
	evidenceGenerator.moveOn(); // move generator so that evidence can be obtained.
    }

    /** Implements method used by {@link ParticleFilterRunner} to obtain evidence for current time step. */
    public Evidence getEvidence() {
	return evidenceGenerator.getEvidence();
    }

    /** 
     * Provides the query instantiations according to current time step,
     * for use by {@link ParticleFilterRunner}. 
     */
    ...
    /** Returns the query obtained by instantiating a query string with the latest time step. */ 
    private ArgSpecQuery getQueryForLatestTimestep(String queryString) {
	return DBLOGUtil.getQueryForTimestep(queryString, model, evidenceGenerator.lastTimeStep);
    }
    ...
    /** An event handler called after every move, with the collection of instantiated queries as argument,
     * with a default that prints the value of all queries on the generator current partial world,
     * including the ones used to obtain evidence. */
...
//	Collection queryStrings = Util.list("#{Blip r: Time(r) = t & Source(r) = MyAircraft}");
	...
	new ParticleFilterRunnerOnGenerator(
		Model.readFromFile(modelFile), linkStrings, queryStrings, properties)
	.run();
    }
}
