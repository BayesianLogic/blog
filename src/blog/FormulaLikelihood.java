...
/**
 * This is a collection of methods aimed at computing
 * the likelihood of formulas without sampling their components
 * (atomic and equality formulas),
 * making it exact given the parents of these components.
 * This is not always possible, however.
 * It is possible when the formula is a <i>mx-DNF</i>,
 * that is, a DNF in which all disjuncts are mutually exclusive,
 * in which all random variables present are independent from each
 * other given their parents (in other words, when there is no
 * parent-child relationship between any of them).
 * Since deciding this independence for all possible instantiations of parents
 * can be complicated,
 * we for now consider particular supporting instantiations of parents,
 * making the notion of mx-DNF relative to an actual partial world. 
 * 
...
public class FormulaLikelihood {
     ...
     */
    public static boolean isDNFWithMutuallyExclusiveDisjuncts(DisjFormula disjunction) {
     ...
     */
    public static boolean areMutuallyExclusive(ConjFormula conjunction, ConjFormula another) {
	// two conjunctions will be mutually exclusive if at least one conjunct in one of them
	// implies the negation of a conjunct of the other.
	// Since that relation is symmetric, there is no need to test it both ways.
	List cartesianProduct = new common.CartesianProduct(Util.list(conjunction.getConjuncts(), another.getConjuncts()));
	for (Iterator it = cartesianProduct.iterator(); it.hasNext(); ) {
	    List pair = (List) it.next();
	    if (areMutuallyExclusiveLiterals((Formula)pair.get(0), (Formula)pair.get(1)))
		return true;
	}
	return false;
    }
    ...
    public static boolean implyTheNegationOfEachOther(EqualityFormula e1, EqualityFormula e2) {
	...
	return
	isVariable(a) && isVariable(b) && a.equals(b) &&
	!isVariable(c) && !isVariable(d) && !c.equals(d);
    }
	...
	! ((NonRandomFunction)f).getInterpClass().equals(ConstantInterp.class);
    }
	...
	if (literal1 instanceof EqualityFormula) {
		...
		if (negFormula2.getNeg() instanceof EqualityFormula)
		    return implyTheNegationOfEachOther
		    ((EqualityFormula) literal1, (EqualityFormula) negFormula2.getNeg());
	    }
	    return false;
	}
	
	Util.fatalError("Formula passed to FormulaLikelihood.literalImpliesLiteral not a literal: " + literal1);
	return false;
    }
    
    public static boolean implies(EqualityFormula equality1, EqualityFormula equality2) {
	return
	(equality1.getTerm1().equals(equality2.getTerm1()) && equality1.getTerm2().equals(equality2.getTerm2()))
	||
	(equality1.getTerm1().equals(equality2.getTerm2()) && equality1.getTerm2().equals(equality2.getTerm1())); 
    }
}
