...
public class Gnuplot {
    /**
     * Receives data (<code>xSeries</code> is a {@link NullaryFunction}, returning a list of numbers,
     * or <code>null</code>,
     * and <code>ySeries</code> is a list of {@link #YSeries}),
     * and calls gnuplot on them, first issues a list of pre-commands to it.
     * If one of the precommands is "persist", gnuplot persists after execution.
     */
    static public void plot(Collection precommands, NullaryFunction xSeries, List ySeriesList) {
	...
	try {
	    ...
	    Util.wait_NE(gnuplotLaunchTime); // make sure this process lasts long enough for gnuplot to do read temp files before they are deleted.
	}
	catch (IOException e) { Util.fatalError("Could not pipe commands to gnuplot.", e); }
	data.cleanUp();
    }
	...
	for (Iterator it = precommands.iterator(); it.hasNext();) {
	    ...
	    if (precommand.equals("persist")) continue;
	...
	private String getDescription(NullaryFunction xSeries, YSeries ySeries) throws IOException {
	    ...
	    if (titleClause.equals("")) titleClause = extractClause(ySeries.directives, "notitle", "notitle");
	    String withClause = extractClause(ySeries.directives, "with ", "w ");
	    String path = storeDataAndReturnPath(xSeries == null? null : (Iterator)xSeries.evaluate(), (Iterator)ySeries.data.evaluate());
	...
	private String extractClause(List directives, String fullName, String abbreviation) {
	    String clause = "";
	...
	private String storeDataAndReturnPath(Iterator xSeries, Iterator ySeries) throws IOException {
//	    File tempFile = File.createTempFile("gnuplot", ".dat");
     ...
     * Their order does not matter and they are correctly placed in the gnuplot command.
     */
