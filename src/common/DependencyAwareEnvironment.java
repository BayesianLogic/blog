 ...
 * This is useful if we want to use the map as a cache for multiple variables
 ...
 * so it is recalculated next time it is used.
 * <p>
 * This is guaranteed by, every time a function on variables is calculated,
 * this being done so via the method {@link #getResultOrRecompute(DAEFunction)},
 * where the {@link DAEFunction} provided recalculates the quantity from the environment,
 * which is passed as its only argument.
 * If a function's {@link DAEFunction#isRandom()} returns <code>true</code>,
 * or an ancestor function is random, it is always recomputed.
 * <p>
 * The environment keeps track of dependencies and only recalculates functions when this is really needed.
 * @author Rodrigo
 */
public class DependencyAwareEnvironment extends TreeMap<String, Object> {
	...
	return get(variable); // calling get ensures that variable is recorded as parent of whatever is being watched now.
    }
    ...
    public Object get(String variable) {
        if ( ! watchedVariables.isEmpty()) {
            ...
            //System.out.println(watchedVariables.peek() + " recorded as child of " + variable);
    ...
    private void finishedCalculation(String variable, Object value) {
        if ( ! watchedVariables.isEmpty() && ! watchedVariables.peek().equals(variable))
            Util.fatalError("DependencyAwareEnvironment.finishedCalculation called on " + variable + " when " + watchedVariables.peek() +  " is the currently calculated variable.");
        put(variable, value);
