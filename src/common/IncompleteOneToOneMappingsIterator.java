...
/**
 * Iterator ranging over maps representing mappings between two given sets.
 * Each mapping involves one-to-one pairs, but may leave (no or all) elements
 * on either side unmapped. 
 * @author Rodrigo
 */
public class IncompleteOneToOneMappingsIterator extends EZIterator {
	...
	if (aFirst != null) { // if a was not empty
	    if (aFirstElementNextMappeeIndex == -1) { // first go through cases with no mapping for aFirst
		if (subIterator == null) {
	...
	Iterator it = new IncompleteOneToOneMappingsIterator(
		Util.list("x", "y", "z", "w"),
		Util.list(1, 2, 3, 4));
	int i = 0;
	while (it.hasNext()) {
