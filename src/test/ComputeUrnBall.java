...
public class ComputeUrnBall {
	...
	public static void print(BigFraction[] probN) {
		for (int i=1; i < probN.length; i++) {
			System.out.print(i);
			System.out.print("\t");
			System.out.println(new BigDecimal(probN[i].getNumerator()).divide(new BigDecimal(probN[i].getDenominator()), 100, RoundingMode.HALF_UP).doubleValue());
		}
	}
	...
	public BigFraction[] compute(int[] obs) {
		...
		for (int N = 1; N <= maxN; N++) {
			...
			for (int k = 0; k <= N; k++) {
				// * 1/ (2^N) * (N! / k! / (N-1)!)
				// choice(N, k) * N
				totalnum = totalnum.add(choice(N, k).multiply(new BigFraction(N).pow(obs.length)));
				...
				for (int i = 0; i < obs.length; i++) {
					if (obs[i] == 0) {
						tmpres = chose0.multiply(ProbCorrectObs).add(chose1.multiply(ProbWrongObs));
					} else {
						tmpres = chose1.multiply(ProbCorrectObs).add(chose0.multiply(ProbWrongObs));
					}
					p = p.multiply(tmpres);
				}
