 ...
 * 3) Define setInferenceProps, adding arguments for the inference engine to
 * inferenceProps.
 * 4) Define referenceProbs, generating "correct" probability tables to compare
 * against the model.
 * 5) Define main, constructing an instance of the test class and calling
 * runTest() on it.
 * 
    ...
    for (Iterator<Query> modelIter = queries.iterator(); modelIter.hasNext();) {
      ...
      if (!(q instanceof ArgSpecQuery)) {
        throw new IllegalStateException(
            "Query has no histogram (not ArgSpecQuery)");
      }
